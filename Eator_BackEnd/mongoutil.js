...
const getDocs = async (dbName, collName, query={}, sortfield={},pageNumber=1,nPerPage=10)=>{
  // create our client
  const client  = await createClient()
  // create/fetch the DB
  const db = client.db(dbName)
...
const getDoc = async (dbName, collName, query={}, sortfield={})=>{
  // create our client
  const client  = await createClient()
  // create/fetch the DB
  const db = client.db(dbName)
  ...
  const coll = db.collection(collName)
  // insert the doc
  const result = await coll.findOne(query).sort(sortfield)
  // return the result
  return result
}
